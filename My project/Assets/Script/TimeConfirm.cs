...
public class TimeConfirm : MonoBehaviour
{
    ...
    void Update()
    {
        if(SuddenDeath.isSuddenDeath){
            ...
            GameTimer.timeRemaining = GameTimer.SuddenDeathTime;
            isPaused.isPausing = true;
        ...
        else if (GameTimer.timeRemaining <= 0.0f && GameTimer.timeRemaining > -1.0f)
        {
            GameSet();
        }else if(GameTimer.timeRemaining <= -1.0f){
            //カウントダウンが終わったら，カウントダウンを非表示にする
            Countdown.gameObject.SetActive(false);
        }
    }
